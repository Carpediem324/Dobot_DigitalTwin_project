...
def generate_launch_description():
    ...
    default_model_path = urdf_tutorial_path / 'model/magician_standalone_with_rs.urdf.xacro'
    default_rviz_config_path = urdf_tutorial_path / 'rviz/urdf_with_rs.rviz'
    ...
    tool_arg = DeclareLaunchArgument(name = 'tool', default_value = 'suction_cup', 
...
'---------------------------------------------------------------------------')
    ...
    rs_recog_node = ExecuteProcess(
        cmd = ['ros2', 'run', 'interate_prac', 'gd'],
        shell = True,
        output = 'screen',
    )

    pp = ExecuteProcess(
        cmd = ['pip', 'install', 'numpy==1.23.0', 'setuptools==58.2.0'],
        shell = True,
        output = 'screen',
    )

    # Dobot bringup launch: Dobot 제어 시스템 활성화
    dobot_bringup_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join('/home/ubuntu/magician_ros2_control_system_ws/src/dobot_bringup/launch', 'dobot_magician_control_system.launch.py')
        )
    )
    ...
    return LaunchDescription([
        ...
        TimerAction(period=5.0, actions=[
                                        ...
                                        rviz_node,
                                        config_info,]),
        TimerAction(period=40.0, actions=[rs_recog_node]),
    ])
